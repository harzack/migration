...
// start writing in the error copy files log:
var ErrorFileCopy = "./destination/fileNotCopied_" + cuttOffDate + "_" + iterationRun + ".txt";
fs.writeFile(ErrorFileName, 'List of files not copied: \r\n', function (err) {
	if (err) {
		return console.log(err);
	}
});
...
/* 		fs.createReadStream(fSource).pipe(fs.createWriteStream(fDestination));
		...
		try {
			...
			console.log(fSource + ' was copied to destination...');
		}
		catch(err) {
			//console.log(err);
			var copyFileObject = {
			...
			fileCopyError.push(copyFileObject);
			totalfileNotCopied += 1;
		} */
...
function minVersion(aDocument) {
	var versMin = 100;
	for (var iDocs = 0; iDocs < aDocument.length; iDocs++) {
		if (aDocument[iDocs].version < versMin) {
			versMin = aDocument[iDocs].version;
		}
	}
	return versMin;
}
...
// this function builds the list of files for one documents with proper data to build the import xml
function buildDocList(documents) {
	...
	var docMinVersion = parseInt(minVersion(documents));
	...
	for (var docs = 0; docs < documents.length; docs++) {
		...
		if (doNotimportList.indexOf(Number(documents[docs].docnumber)) == -1) {
			...
			if (oneDocument.file != "noFile" && parseInt(oneDocument.version) == docMinVersion) {
				oneDocument.minVersion = docMinVersion;
				// console.log("good min version: " + oneDocument.minVersion);
			} else {
				docMinVersion += 1;
				// console.log("calculated min version: " + docMinVersion );
			}
			...
			if ( oneDocument.title.length > 230) {
				oneDocument.title = oneDocument.title.slice(0,230) + './.';
			}			
...
function buildXML(documents) {
	...
	for (var iDoc = 0; iDoc < documents.length; iDoc++) {
		...
		if (documents[iDoc].file != 'noFile') { // we found a corresponding file
			...
			if (documents[iDoc].version == documents[iDoc].minVersion) {
...
console.log("Total number of documents to process: " + totalDocs + " with a total of: " + FListLen + " files (versions).");
//console.log("list of document numbers to process: " + uniqueDocNumberList);
for (var udocs = 0; udocs < totalDocs; udocs++) {
	// progress lines...
	// process.stdout.write('\x1Bc');
	//console.log('...processing document ' + (udocs + 1) + ' of ' + totalDocs);
...
fs.appendFile(ErrorFileCopy, JSON.stringify(fileCopyError, null, 2), function (err) {
	if (err) {
		return console.log(err);
	}
	console.log("the list of files not copied is saved! - number of files not copied: " + totalfileNotCopied);
});

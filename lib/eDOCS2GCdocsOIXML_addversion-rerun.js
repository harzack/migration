...
const CIFPrefix = "\\\\DC03QVN0201.prod.global.gc.ca\\GCDOCS_PROD\\MIGRATION\\DFO\\";
...
function buildDocList(documents) {
	...
	for (var docs = 0; docs < documents.length; docs++) {
		...
		if (doNotimportList.indexOf(Number(documents[docs].docnumber)) == -1) {
			...
			// - destination folder (removes trailing ":" and all of them if multiple colon are staying from the source)
			oneDocument.location = documents[docs].destinationFolder.replace(/:+$/, "");
			// - title: wrap in ![CDATA[]] and append # and document number to all documents imported from RDIMS:
			oneDocument.title = '![CDATA[' + documents[docs].title + ' EKME ' + documents[docs].docnumber + ']]';
			// - ensure there is no colon in the title, replace them with spaces (only forbidden character in CS titles):

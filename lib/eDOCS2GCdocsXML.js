...
var documentsJSON = require('./../source/MCU_extract_008.json');
...
fs.writeFile('./destination/oiGCCMS_001_008.xml', '<?xml version="1.0" encoding="utf-8"?>\r\n', function(err) {
	if(err) {
		return console.log(err);
	}
});
// then the error file:
fs.writeFile('./destination/fileNotFound_008.txt', 'List of doc numbers with missing files: \r\n', function(err) {
	if(err) {
		return console.log(err);
	}
});
...
function buildDocList(documents){
	...
	for (var docs = 0 ; docs < documents.length ; docs++) {
		...
		oneDocument.versiondesc = documents[docs].versiondesc;
		...
		if (documents[docs].description == "") {
			oneDocument.description = 'Imported from EKME '
...
function buildXML(documents) {
	...
	for (var iDoc = 0; iDoc < documents.length; iDoc++) {		
		...
		if (documents[iDoc].file != 'noFile'){ // we found a corresponding file
			...
			if (documents[iDoc].version == documents[iDoc].minVersion && documents[iDoc].subversion == "!") { // first version
				var nodeObject = {
						node: {
							...
							categoryGCCMS: {
								attribute: builGCCMSnode(documents[iDoc].GCCMS),
			...
			// need to check if "Add Title to Location” check box is selected in settings (then this code is fine), 
			// otherwise need to append the title to location and remove <title>
...
fs.appendFile('./destination/oiGCCMS_001_008.xml', xmlObjectImporter.toString({ pretty: true }), function(err) {
	if(err) {
		return console.log(err);
	}
...
fs.appendFile('./destination/fileNotFound_008.txt', JSON.stringify(fileNotFound, null, 2), function(err) {
	if(err) {
		return console.log(err);
	}

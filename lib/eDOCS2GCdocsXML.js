...
- MCU_extract_00x.json contains all EKME metadata extracted from the database for all documents and their versions we need to import.
	structure:
...
var JSONSourceFileName = "./../source/MCU_extract_" + iterationRun + ".json";
...
var DestinationFileName = "./destination/oiGCCMS_001_" + iterationRun + ".xml";
...
function buildDocList(documents){
	...
	for (var docs = 0 ; docs < documents.length ; docs++) {
		...
		if (documents[docs].description == "") {
			...
			for(gccmsCatlen = 0; gccmsCatlen < numberOfGccmsDockets; gccmsCatlen++) {

...
var docNumbers = {
	...
	Num2DOCSunix: function() {
		...
		for (var i=0; i< this._numbers.length; i++){
			var curNumber = this._numbers[i];
			var unixSubVersion;
			if (this._subversions[i] == "!") {
				unixSubVersion = "_";
			} else {
				unixSubVersion = this._subversions[i];
			}
			filenames[i] = "";
			while(curNumber >= 1){
			...
			filenames[i] = filenames[i].toLowerCase();
			filenames[i] = reverse(filenames[i])+pad2(this._versions[i])+unixSubVersion+"."+this._extensions[i];
		}
		return filenames;
	},
	reset: function(){
...
var docFiles = {
	...
	extractExtensions: function (){
		...
		for (var i=0; i< this._fileNames.length; i++){
            var extPos = this._fileNames[i].lastIndexOf(".");
	...
	extractNumbers: function (){
		...
		for (var i=0; i< this._fileNames.length; i++){
            var extPos = this._fileNames[i].lastIndexOf(".");
            var fileMethod = this._fileNames[i].slice(extPos-1, extPos);
			if (fileMethod === "!"){
...
function DOCSEnh2Num(filename) {
        ...
        for (var j = 1;  j < filename.length+1; j++){
        	docnum *= 36;
